...
- Use only erasable TypeScript syntax (Node strip-only mode) in code checked by the root config (`packages/*/src`, `packages/*/test`, `app/test`): no parameter properties, `enum`, `namespace`/`module`, `import =`, `export =`, or other constructs needing JS emit. Use explicit fields with constructor assignments.
...
- Never run the full vitest suite directly: it includes e2e tests that activate when endpoint/auth env vars are present. For all non-e2e tests, run `./test.sh` from the repo root. Otherwise run specific tests from the package root:
  ...
  - `app` (`node:test`): `node --test test/specific.test.ts`
- If you create or modify a test file, run it and iterate on test or implementation until it passes.
- For `app/test/suite/`, use `test/suite/harness.ts` + the faux provider. No real provider APIs, keys, or paid tokens.
...
- Message format: `{feat,fix,docs}[(agent-core,workspace,source-control,app)]: <commit message> (optionally multiple lines)`. Message is informative and concise.
...
- Add `pkg:*` labels for affected packages (`pkg:agent-core`, `pkg:workspace`, `pkg:source-control`); use all that apply.
